...
function displayProduct(products){
    ...
    for (i = 0; i < panier.length; i++) {
        ...
        // limite le nombres d'article a 100
        if (panier[i].nombre > 100) {
            panier[i].nombre = 100;
            savePanier(panier);
        }
    ...
    function totalArticle(totalPrice) {
        ...
        totalPrice.textContent = `${value_totalPrix}`;
    }
    totalArticle(totalPrice);
};
...
// *******finFONCTION total prix produits
...
// *******debut FONCTION pour virer le background apres un  setTimeout
function setTim(arg){
